...
@test "11.Check root-ca certificatee " {    
    ...
    assert_output "subject=DC = grao, DC = asir2, O = IES Grao Inc, CN = IES Grao Root CA"
...
@test "15.Check certificate signing request info file" {            
    egrep "^DNS\..+= apache\.$username\.asir2.grao$" ~/certs/etc/apache.$username.request.asir2.grao.conf
...
@test "27.Check certificates from own server" {
    ...
    assert_line "issuer=DC = grao, DC = asir2, O = IES Grao Inc, CN = IES Grao Root CA"    
